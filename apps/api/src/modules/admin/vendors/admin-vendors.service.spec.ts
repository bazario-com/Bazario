...
describe('AdminVendorsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [AdminVendorsService, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  it('rejects re-approving an already-approved vendor', async () => {
    ...
    await expect(service.approve('v1')).rejects.toThrow(BadRequestException);
  });

  it('approving a vendor sets status APPROVED and clears any prior rejection reason', async () => {
    prisma.vendor.findUnique.mockResolvedValue({ id: 'v1', status: 'PENDING' });
    prisma.vendor.update.mockResolvedValue({ id: 'v1', status: 'APPROVED' });

    await service.approve('v1');
  ...
  it('rejecting a vendor records the reason', async () => {
    ...
    await service.reject('v1', 'Incomplete business documents');
